...
def simple_interest(principal, rate, time):
    ...
    return(principal * (rate / 100) * time) / 100
...
def convert_temperature(temp, unit):
    ...
    if unit == 'C':
        return((temp * 9/5) + 32,'Fahrenheit')
         ...
         return(round((temp + 32) * 5/9),'Celcius')
...
print('Exercise 6:', largest(1, 210, 3))
...
def basicCalculator(A1, B2, Sign):
    if Sign == 'add':
        return A1 + B2
    elif Sign == 'subtract':
        return A1 - B2
        ...
        return f"Can't perform operation. {A1} & {B2} has no symbol"

print('Exercise 9:', basicCalculator(10, 5, "add"))
